...
pub fn render(f: &mut Frame<'_>, app: &App, area: Rect) {
    ...
    let input_line = Line::from(vec![
        ...
        Span::styled(&app.package_picker_input, Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
    ...
    f.render_widget(
        Paragraph::new(Line::from(Span::styled(divider, Style::default().fg(Color::Rgb(50, 50, 65))))),
        parts[1],
    );
    ...
    // "All packages" entry
    {
        ...
        items.push(ListItem::new(Line::from(Span::styled(
            ...
            Style::default()
                .fg(if sel { Color::Black } else if active { Color::Green } else { Color::White })
    ...
    for (i, pkg) in filtered.iter().enumerate() {
        ...
        let row_spans: Vec<Span> = row_spans
            ...
            .map(|s| {
                if sel {
                    Span::styled(s.content.into_owned(), Style::default().fg(Color::Black).bg(bg).add_modifier(Modifier::BOLD))
                } else {
                    s
                }
            })
